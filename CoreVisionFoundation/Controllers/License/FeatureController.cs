...
namespace CoreVisionFoundation.Controllers.License
{
    ...
    public class FeatureController : ApiControllerWithOdataRoot<FeatureSM>
    {
        ...
        #region Add Endpoint
        [HttpPost]
        [Authorize(AuthenticationSchemes = APIBearerTokenAuthHandler.DefaultSchema, Roles = "SystemAdmin,SuperAdmin")]
        public async Task<ActionResult<ApiResponse<FeatureSM>>> Post([FromBody] ApiRequest<FeatureSM> apiRequest)
        {
            ...
            if (FeatureSM != null)
            {
                return CreatedAtAction(nameof(FeatureController.GetById), new
                {
                    id = FeatureSM.Id
