...
namespace CoreVisionFoundation.Controllers.License
{
    ...
    public class PaymentController : ApiControllerRoot
    {
        ...
        public async Task<ActionResult<ApiResponse<CheckoutSessionResponseSM>>> StripeWebHook()
        {
            ...
            var resp = await this._paymentProcess.RegisterStripeWebhook(json, Request.Headers["Stripe-Signature"], _apiConfiguration.ExternalIntegrations.StripeConfiguration.WHSecret);
            if (resp != null)
            {
                if (resp)
                    return Ok();
