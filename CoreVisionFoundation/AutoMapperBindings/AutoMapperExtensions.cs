...
namespace CoreVisionFoundation.AutoMapperBindings
{
    public static class AutoMapperExtensions
    {
        public static AutoMappingResponse RegisterAutoMapperFromDmToSm<DmT, SmT>(this IProfileExpression expression, Action<IMappingExpression, Type, Type> afterAutoMapAction = null, string dmNameExtension = "DM", string smNameExtension = "SM", Action<IMappingExpression, Type, Type> additionMapAction = null)
        {
            ...
            Func<Type, bool> filter = (Type x) => x.IsClass && !x.IsAbstract && x.Name.EndsWith(dmNameExtension2) && x.IsAssignableTo(typeof(DomainModelRootBase)) && x.GetCustomAttributesData().FirstOrDefault((CustomAttributeData y) => y.AttributeType == typeof(IgnoreClassAutoMapAttribute)) == null;
            expression.CreateMapsForSourceTypes(typeof(DmT).Assembly, filter, delegate (Type srcType)
            {
                ...
                if (type != null && type.IsAssignableTo(typeof(BaseServiceModelRoot)) && type.GetCustomAttributesData().FirstOrDefault((CustomAttributeData y) => y.AttributeType == typeof(IgnoreClassAutoMapAttribute)) == null)
                {
                    return type;
                }
        ...
        public static IMappingExpression IgnorePropertiesFromConversionIfMarked(this IMappingExpression expression, Type sourceType, Type destinationType)
        {
            ...
            List<string> list = (from x in sourceType2.GetProperties()
                                 where x.CustomAttributes.Any((CustomAttributeData y) => (y.AttributeType == typeof(IgnorePropertyOnReadAttribute) && IsConversionValid(y, sourceType2, destinationType2)) ? true : false)
            ...
            List<string> list2 = (from x in destinationType2.GetProperties()
                                  where x.CustomAttributes.Any((CustomAttributeData y) => (y.AttributeType == typeof(IgnorePropertyOnWriteAttribute) && IsConversionValid(y, sourceType2, destinationType2)) ? true : false)
                                  select x.Name).ToList();
        ...
        public static IMappingExpression HandleLocalDateTimeConversionProperties(this IMappingExpression expression, Type sourceType, Type destinationType)
        {
            ...
            {
                CustomAttributeData customAttributeData = propertyInfo.CustomAttributes.Where((CustomAttributeData x) => x.AttributeType == typeof(ConvertFromUtcToLocalDateAttribute)).FirstOrDefault();
                ...
                dictionary.Add(propertyInfo.Name, customAttributeData.NamedArguments?.FirstOrDefault((CustomAttributeNamedArgument z) => z.MemberName == "SourcePropertyName").TypedValue.Value?.ToString());
            }
        ...
        public static IMappingExpression HandleFilePathToUriProperties(this IMappingExpression expression, Type sourceType, Type destinationType)
        {
            ...
            foreach (PropertyInfo propertyInfo in properties)
            {
                CustomAttributeData customAttributeData = propertyInfo.CustomAttributes.Where((CustomAttributeData x) => x.AttributeType == typeof(ConvertFilePathToUriAttribute)).FirstOrDefault();
                if (customAttributeData == null)
                {
                    continue;
                }
