using CoreVisionBAL.Foundation.Config.Dependency;
...
namespace CoreVisionBAL.Foundation.Web
{
    public static class Extentions
    {
        public static int GetUserRecordIdFromCurrentUserClaims(this ClaimsPrincipal claimsPrincipal)
        {
            ...
            {
                Claim claim = (claimsPrincipal.Identity as ClaimsIdentity).Claims.FirstOrDefault((Claim x) => x.Type == "dbRId");
                if (claim != null)
                {
        ...
        public static string GetUserRoleTypeFromCurrentUserClaims(this ClaimsPrincipal claimsPrincipal)
        {
            if (claimsPrincipal != null && claimsPrincipal.Identity != null && claimsPrincipal.Identity.IsAuthenticated && claimsPrincipal.Identity is ClaimsIdentity)
            {
                Claim claim = (claimsPrincipal.Identity as ClaimsIdentity).Claims.FirstOrDefault((Claim x) => x.Type == "http://schemas.microsoft.com/ws/2008/06/identity/claims/role");
                if (claim != null)
                {
                    return claim.Value;
                }
        ...
        public static string GetUserEmailFromUserClaims(this ClaimsPrincipal claimsPrincipal)
        {
            if (claimsPrincipal != null && claimsPrincipal.Identity != null && claimsPrincipal.Identity.IsAuthenticated && claimsPrincipal.Identity is ClaimsIdentity)
            {
                Claim claim = (claimsPrincipal.Identity as ClaimsIdentity)?.Claims.FirstOrDefault((Claim x) => x.Type == "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress");
                if (claim != null)
                {
                    return claim.Value;
                }
        ...
        public static string GetCompanyCodeFromCurrentUserClaims(this ClaimsPrincipal claimsPrincipal)
        {
            if (claimsPrincipal != null && claimsPrincipal.Identity != null && claimsPrincipal.Identity.IsAuthenticated && claimsPrincipal.Identity is ClaimsIdentity)
            {
                Claim claim = (claimsPrincipal.Identity as ClaimsIdentity).Claims.FirstOrDefault((Claim x) => x.Type == "clCd");
                if (claim != null)
                {
                    return claim.Value;
                }
        ...
        public static int GetCompanyRecordIdFromCurrentUserClaims(this ClaimsPrincipal claimsPrincipal)
        {
            if (claimsPrincipal != null && claimsPrincipal.Identity != null && claimsPrincipal.Identity.IsAuthenticated && claimsPrincipal.Identity is ClaimsIdentity)
            {
                Claim claim = (claimsPrincipal.Identity as ClaimsIdentity).Claims.FirstOrDefault((Claim x) => x.Type == "clId");
                if (claim != null)
                {
                    return Convert.ToInt32(claim.Value);
                }
        ...
        public static void AutoRegisterAsSelfFromBaseTypes<BaseT, TargetType>(this IServiceCollection services, ServiceLifetime serviceLifetime)
        {
            ...
            IEnumerable<Type> source = from x in assembly.GetExportedTypes()
                                       where x.IsClass && !x.IsAbstract && typeof(BalRoot).IsAssignableFrom(x) && x.GetCustomAttributesData().FirstOrDefault((CustomAttributeData y) => y.AttributeType == typeof(IgnoreAutoInjectAttribute)) == null
                                       select x;
        ...
        public static string GetValueFromHeaderOrQueryByKey(this HttpRequest request, string key)
        {
            ...
            if (request != null)
            {
                ...
                {
                    KeyValuePair<string, StringValues> keyValuePair = request.Query.FirstOrDefault<KeyValuePair<string, StringValues>>((KeyValuePair<string, StringValues> x) => x.Key == key2);
        ...
        public static async Task<object> ToCustomizedObjectToLog(this HttpResponse response, bool addBody, bool addCookies, bool addSesitiveInfo = false, bool digestException = true, List<string> skipHeaderKeys = null)
        {
            ...
            try
            {
                if (response != null)
                {
                    ...
                    {
                        headers.RemoveAll((KeyValuePair<string, StringValues> x) => skipHeaderKeys2.Contains(x.Key.ToLower()));
                    }

                    headersToLog = headers?.ToDictionary((KeyValuePair<string, StringValues> x) => x.Key, (KeyValuePair<string, StringValues> x) => x.Value.ToString());
                }
            }
            catch (Exception e)
            {
        ...
        public static async Task<object> ToCustomizedObjectToLog(this HttpRequest request, bool addBody, bool addCookies, bool addSesitiveInfo = false, bool digestException = true, List<string> skipHeaderKeys = null)
        {
            ...
            try
            {
                if (request2 != null)
                {
                    ...
                    {
                        formItems = request2.Form.Keys.Where((string a) => !a.ToLower().Contains("password") || !a.ToLower().Contains("usertoken") || !a.ToLower().Contains("card") || !a.ToLower().Contains("cvv") || !a.ToLower().Contains("__VIEWSTATE")).ToDictionary((string x) => x, (string x) => request2.Form[x]);
                    }
            ...
            string url = ((request2 == null) ? "null" : UriHelper.GetDisplayUrl(request2));
            ...
            return new
            {
                ...
                RawUrl = ((httpRequest3 != null) ? UriHelper.GetEncodedPathAndQuery(httpRequest3) : null),
