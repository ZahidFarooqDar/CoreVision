...
{
    public class PaymentProcess : CoreVisionBalBase
    {
        #region Properties
        ...
        public async Task<CheckoutSessionResponseSM> CheckoutSession(CheckoutSessionRequestSM reqData, int customerId)
        {
            ...
            if (user == null)
                throw new CoreVisionException(ApiErrorTypeSM.NoRecord_Log, $"User for id {customerId} not found to create checkout session.", "Error occurred in payment, Please login again.");
            if (!user.IsEmailConfirmed)
                throw new CoreVisionException(ApiErrorTypeSM.NoRecord_NoLog, $"User for id {customerId} email not confirmed.", "Please verify your email in profile section.");
            ...
            {
                throw new CoreVisionException(ApiErrorTypeSM.Fatal_Log, $"{res.ResponseMessage}");
            }
            ...
            {
                throw new CoreVisionException(ApiErrorTypeSM.Fatal_Log, e.Message, "Some error occurred, please try again.", e);
            }
        ...
        {
            var user = await this._clientUserProcess.GetClientUserById(customerId);
            if (user == null)
                throw new CoreVisionException(ApiErrorTypeSM.NoRecord_Log, $"User for id {customerId} not found to get to customer portal.", "Error occured in getting details, Please login again.");
            if (!user.IsEmailConfirmed)
                throw new CoreVisionException(ApiErrorTypeSM.NoRecord_NoLog, $"User for id {customerId} email not confirmed.", "Please verify your email in profile section.");
            /*if (!user.IsPhoneNumberConfirmed)
                throw new CoreVisionException(ApiErrorTypeSM.NoRecord_NoLog, $"User for id {customerId} phone number not confirmed.", "Please verify your contact number in profile section.");*/
        ...
        private async Task<UserLicenseDetailsSM?> AddUpdateStripeCustomer(Customer? customer)
        {
            ...
            {
                throw new CoreVisionException(ApiErrorTypeSM.ModelError_Log, $"{ex.Message}", $"Add or Update failed in user subscription table", ex);
            }
        }
        /// <summary>
