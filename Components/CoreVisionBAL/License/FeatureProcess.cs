...
namespace CoreVisionBAL.License
{
    public class FeatureProcess : CoreVisionBalOdataBase<FeatureSM>
    {
        ...
        public override async Task<IQueryable<FeatureSM>> GetServiceModelEntitiesForOdata()
        {
            ...
            IQueryable<FeatureSM> retSM = await base.MapEntityAsToQuerable<FeatureDM, FeatureSM>(_mapper, entitySet);
            return retSM;
        }
        #endregion Odata
        ...
        public async Task<List<FeatureSM>> GetMyFeatures(int clientUserId)
        {
            ...
            if (userLicense == null)
            {
                throw new CoreVisionException(ApiErrorTypeSM.Access_Denied_Log, $"Access denied for getting permissions for user Id : {clientUserId}", "Access denied. A valid license is required to unlock features.");
            }
        ...
        public async Task<FeatureSM?> UpdateFeature(int _featureIdToUpdate, FeatureSM _featureSM)
        {
            ...
            if (_featureSM != null && _featureIdToUpdate > 0)
            {
                ...
                {
                    throw new CoreVisionException(ApiErrorTypeSM.Fatal_Log, $"Feature not found: {_featureIdToUpdate}", "Feature to update not found, add as new instead.");
                }
            }
