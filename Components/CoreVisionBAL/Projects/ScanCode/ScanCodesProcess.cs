...
namespace CoreVisionBAL.Projects.ScanCode
{
    public class ScanCodesProcess : CoreVisionBalOdataBase<ScanCodesFormatSM>
    {
        ...
        public override async Task<IQueryable<ScanCodesFormatSM>> GetServiceModelEntitiesForOdata()
        {
            ...
            IQueryable<ScanCodesFormatSM> retSM = await base.MapEntityAsToQuerable<ScanCodesFormatDM, ScanCodesFormatSM>(_mapper, entitySet);
            return retSM;
        }
        #endregion Odata
        ...
        public async Task<ScanCodesFormatSM> AddBarcodeFormat(ScanCodesFormatSM objSM)
        {
            ...
            throw new CoreVisionException(ApiErrorTypeSM.Fatal_Log, "Something went wrong while Adding Barcode Format Data");
        }
        #endregion Add
        ...
        public async Task<ScanCodesFormatSM> UpdateBarcodeDetails(int id, ScanCodesFormatSM objSM)
        {
            ...
            if (objDM != null)
            {
                ...
                throw new CoreVisionException(ApiErrorTypeSM.Fatal_Log, $"Something went wrong while Updating Barcode Format Details", "Something went wrong while Updating Barcode Format Details");
            }
            else
            {
                throw new CoreVisionException(ApiErrorTypeSM.Fatal_Log, $"Barcode Format not found: ", "Data to update not found, add as new instead.");
            }
        }
        ...
        public async Task<CodeResponseSM> GenerateCode(GenerateBarcodeSM objSM)
        {
            ...
            {
                throw new CoreVisionException(ApiErrorTypeSM.NoRecord_Log, $"No QR code data found for the provided ID: {objSM.Id}. Please verify the ID and try again.");
            }
            if (!Enum.TryParse(existingData.BarcodeFormat, out BarcodeFormat barcodeFormat))
            {
                throw new CoreVisionException(ApiErrorTypeSM.InvalidInputData_Log, $"Invalid barcode format: {existingData.BarcodeFormat}. Please verify the format.");
            }
            ...
            catch (Exception e)
            {
                ...
                throw new CoreVisionException(ApiErrorTypeSM.Fatal_Log, existingData.ErrorData);
            }
        }
