...
{
    public class AzureAIProcess : CoreVisionBalBase
    {
        #region Properties
        ...
        public async Task<AzureAIResponseSM> TranslateTextAsync(TranslationRequestSM objSM)
        {
            ...
            {
                throw new CoreVisionException(ApiErrorTypeSM.Fatal_Log, "Invalid Language Code", "The provided language is not supported for translation. Please verify and provide a valid language.");
            }
            ...
            throw new CoreVisionException(ApiErrorTypeSM.Fatal_Log, "Failed to translate text after multiple retries", "Failed to translate text after multiple retries");
        }
        ...
        public async Task<AzureAIResponseSM> ExtractTextFromBase64ImageAsync(ImageDataSM objSM)
        {
            ...
            {
                throw new CoreVisionException(ApiErrorTypeSM.Fatal_Log, "Image data cannot be empty", "The image data is empty. Please provide a valid image to extract text.");
            }
            ...
            while (retryCount < maxRetries)
            {
                try
                {
                    ...
                    {
                        throw new CoreVisionException(ApiErrorTypeSM.Fatal_Log, $"{await response.Content.ReadAsStringAsync()}", $"{await response.Content.ReadAsStringAsync()}");
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        throw new CoreVisionException(ApiErrorTypeSM.Fatal_Log, $"Something went wrong while Extracting text from the image... Please try Again", $"Something went wrong while Extracting text from the image... Please try Again");

                    }
                }
            }
            throw new CoreVisionException(ApiErrorTypeSM.Fatal_Log, $"Failed to extract text from image after multiple retries.", $"Failed to extract text from image after multiple retries.");
